...
Rather than build URLs by hand, it is possible to generate URLs based on the
endpoint, using `reverse_url()`.
...
* `name` - A string that will identify the command.
* `handler` - A callable that will be executed when running command.
...
Similarly to the routes `Include`, commands can be nested using `SubCommand`.

### SubCommand

#### Constructor

* `name` - A string that will identify the group of commands. This value will override the value
automatically generated by the class or specified by the `name` class properties.
* `commands` - A list of `Command`s. These will be the commands nested under this command.

#### Properties

* `name` - A string that will identify the group of commands. If not
specified, the `SubCommand` will automatically generate a name based on the class
name by truncating `"Commands"`. For example, `UserCommands` will be under the
command named `user`.
* `commands` - A list of `Command`s or `SubCommand`s. These will be the nested commands.

### Example

The following `app` configuration will have 4 top level commands: `user`,
`override_user`, `import`, and `other_command`.

```python
from apistar import Command, SubCommand
from apistar.frameworks.wsgi import WSGIApp as App

class UserCommands(SubCommand):
    commands = [
        Command('create', create_user),
        Command('delete', delete_user),
    ]

class DataCommands(SubCommand):
    name = 'import'
    commands = [
        Command('csv', import_csv),
        Command('yaml', import_yaml),
    ]

commands = [
    UserCommands(),
    DataCommands(),
    Command('other_command', other_command),

    # The same user commands will be accessible
    #under the `override_users` SubCommand.
    UserCommands('override_user'),
]
...
Commands are documented by the doc string on the extended `SubCommand` class and
command functions. Both follow the same format of a short description on the first
line, an empty line, and a longer description starting on the third line. The
longer description can be any length. Failure to separate the short and long
description with an empty line will result in only the long description being
specified.

```python
def my_command():
	""" A short description of the my_command command

	A longer description of the utility of these sub commands can be places
    here and shown when `--help` is called on the `user` SubCommand.
	"""
	print("Hello World!")

class UserCommands(SubCommand):
    """A short description of the user commands
...
```
$> apistar --help
Usage: app.py COMMAND [OPTIONS] [ARGS]...
...
Commands:
  ...
  my_command  A short description of the my_command command
  user        A short description of the user commands


$> apistar my_command --help
Usage: apistar my_command [OPTIONS]

  A longer description of the utility of these sub commands can
  be places here and shown when `--help` is called on the `user` SubCommand.
