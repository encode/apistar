...
* `name` - A string that will identify the group of commands. This value will override the value automatically generated by the class or specified by the `name` class properties.
* `commands` - A list of `Command`s. These will be the commands nested under this command.

#### Properties

* `name` - A string that will identify the group of commands. If not specified, the `SubCommand` will automatically generate a name based on the class name by truncating `"Commands"`. For example, `UserCommands` will be under the command named `user`.
* `commands` - A list of `Command`s or `SubCommand`s. These will be the nested commands.

### Example

The following `app` configuration will have 4 top level commands: `user`, `override_user`, `import`, and `other_command`.
...
Commands are documented by the doc string on the extended `SubCommand` class and command functions. Both follow the same format of a short description on the first line, an empty line, and a longer description starting on the third line. The longer description can be any length. Failure to separate the short and long description with an empty line will result in only the long description being specified.
